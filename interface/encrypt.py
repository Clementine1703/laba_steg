...
class Encrypt(tk.Tk):
    def __init__(self) -> None:
        ...
        buttons.create_button(self, 'Показать ПСП', self.show_psp)
    ...
    @staticmethod
    def show_psp():
        if (settings.psp_list != []):
            files.call_message(f'Длина ПСП: {len(settings.psp_list)}')
        else:
            files.call_message('Последовательность еще не сгенерирована!')
